...
class claim_model extends CI_Model {
	...
	public function claim($item_id,$username){
		$sql = "insert into claimed_list (food_id,user_id,claimed,picked,released) values ($item_id,\"$username\",true,false,false)";
		$this->db->query($sql);
	}
	
	public function release($item_id,$username){
		$sql = "update claimed_list set release=true where item_id=$item_id and user_id=\"$username\"";
		$this->db->query($sql);
	}
	
	public function pickup($item_id,$username){
		$sql = "update claimed_list set picked=true where item_id=$item_id and user_id=\"$username\"";
	...
	public function claim_list($username){
		$sql = "select food_list.food_id,food_name,description,notes,quantity,perishable,expiration_date from food_list,claimed_list,users where claimed_list.user_id=\"$username\" and food_list.food_id = claimed_list.food_id and claimed_list.claimed=1 and users.hid=food_list.user_id";
		$query = $this->db->query($sql);
		if($query->num_rows()==0){
			return;
		}
